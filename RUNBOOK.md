...
| **現在做到哪?** | ★★ **W06 收工(2026-09-12)**。**G1 完成**:逐欄位文件 7 個訊息對、**164 個數字由 CI 從 capture 重算**。**G2 完成**:自己簽的三層鏈 ＋ **Table 1**,五種篡改、十條受控臂。★★ **G3 上半做完了**:那一格「改裝置上的量測值、握手照樣完成、一個錯誤碼都沒有」的篡改,現在被**參考值比對判成 FAIL**,而且 CI 有一個叫 `rats` 的 job 專門在斷言這件事——它變紅的唯一理由就是「被篡改的量測沒有被拒絕」(§11.5)。剩下版本回滾規則,W07 |
...
| G3 | RATS 驗證流水線 | 🟡 **進行中**(W06 做完上半,W07 收尾)—— 參考值、COSE 簽章背書、政策、判定,十條臂全跑過一遍(**Table 3**,`docs/rats-pipeline.md`)。**那個沒有任何一層擋得住的篡改,現在被擋下來了,而且指得出是哪一條規則、哪一個 index。** 還沒做的是回滾規則:目前版本號比的是「完全相等」,所以升版跟降版的判定一模一樣,那是 W07 的四個案例 |
| G4 | 後量子成本 | ⬜ 未開始(W08) |
| G5 | 真實傳輸 | ⬜ 未開始(W09) |
| G6 | 一致性與負面測試 | ⬜ 未開始(W10~W11) |
| G7 | 上游貢獻 | 🟡 **進行中** — 環境已備妥;**第一個 patch 已經備好、還沒送出**:`CoRimTool.py` 的 `verify` 根本沒有在驗簽章,而且是兩行互相遮蔽的缺陷——只修看起來明顯的那一行,會把「什麼都不接受」變成「什麼都接受」。分支、commit、PR 內文都寫好了,按送出的那一下是他的(`docs/upstream/0001-corim-verify.md`)。十二個候選有證據,送出去的是零 |
