#!/bin/bash
# Generated by harness/run_afmctp.sh on 2026-09-17T19:11:46Z.
...
note "attach the line discipline in each namespace"
ip netns exec A $MCTP link serial "${PTS[0]}" > "$OUT/linkA.log" 2>&1 &
LA=$!
ip netns exec B $MCTP link serial "${PTS[1]}" > "$OUT/linkB.log" 2>&1 &
LB=$!

wait_iface() {
    local ns=$1 i
    for i in $(seq 80); do
        if ip netns exec "$ns" $MCTP link show 2>/dev/null \
             | awk '$1=="dev" && $2 ~ /^mctpserial/ {found=1} END{exit !found}'; then
            return 0
        fi
        sleep 0.1
    done
...
wait_iface A || { fail "no mctpserial interface appeared in namespace A"; cat "$OUT/linkA.log"; exit 3; }
wait_iface B || { fail "no mctpserial interface appeared in namespace B"; cat "$OUT/linkB.log"; exit 3; }

IF_A=$(ip netns exec A $MCTP link show | awk '$1=="dev" && $2 ~ /^mctpserial/ {print $2; exit}')
IF_B=$(ip netns exec B $MCTP link show | awk '$1=="dev" && $2 ~ /^mctpserial/ {print $2; exit}')
echo "A: $IF_A    B: $IF_B"
