...
| 11 | an upstream change with reviewer correspondence | G7 | **both halves exist, 2026-09-22, and neither is finished** — the change exists twice, [`DMTF/spdm-emu` #524](https://github.com/DMTF/spdm-emu/pull/524) and [`openbmc/spdm` 94773](https://gerrit.openbmc.org/c/openbmc/spdm/+/94773), and the correspondence exists once: two inline comments from a reviewer 35 minutes after the push, then a −1 from an owner the same evening. Patchset 2 was pushed on 2026-09-23 and the same owner answered it within six minutes; patchset 3 was sent the same day. Nothing is merged. The external clock is why this gate started in week one, and it is still the thing that decides when it closes |
| 12 | `docs/threat-scope.md` — what is and is not defended against | G8 | **complete**, 2026-09-23 — the day-one outline is still there, with libspdm's own five-layer threat model, the implementation-layer section of 2026-09-20, and week 12's half: five things SPDM gives a requester and whether each was observed here, six it does not and what does, and the blind spot — the three 2026 advisories were C and a specification's text, not cryptography. Its level-2 row said *nothing in this project has ever established a secure session*; two committed captures that had never been decoded say otherwise, and the row now says so |
| 13 | `docs/limitations.md` | G8 | **complete**, 2026-09-23 — six layers, every entry labelled *not done* or *limit*, every `TODO(W12)` filled from evidence rather than from memory, and the scope entry corrected where it had been true only of the captures that were decoded |
...
21. **A claim about every capture is a claim about the captures that were
    read.** Week twelve found seven documents, and the reason one tool gave for
    an advisory verdict, saying that no secure session had ever been
    established here. Each was checked against every committed *decode*, and 20
    of the 152 committed captures had none; two of those 20 hold completed
    sessions, one of them in the week-1 run the README cites by name. A census
    has to say what it did not count, so `harness/census.sh` reads every
    capture and names the ones `spdm_dump` cannot, `verify_repo.sh` fails when a
    capture arrives that nothing reads, and `check_advisories.py` names the
    captures whose encrypted records it cannot see instead of reporting that a
    request was never sent. The same week's other instance was a record about a
