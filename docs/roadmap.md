...
| 9 | negative test suite reproducing three advisory classes | G6 | reproducible — **specified, not written.** `negative/` holds three files whose header comments are their specifications; all three compile under `-Werror` with both sanitizers and `DONE.txt` is empty, so CI runs none of them and claims none of them. W11 |
...
| 12 | `docs/threat-scope.md` — what is and is not defended against | G8 | written — the day-one outline is still there, with libspdm's own five-layer threat model and the implementation-layer section added on 2026-10-12. The row that matters is level 2: **nothing in this project has ever established a secure session**, and it took a fuzz corpus to notice |
