...
# ── the controlled set, and the six algorithm groups ───────────────────────
#
# Moved to harness/lib/arms.sh on 2026-09-18, when harness/run_afmctp.sh began
# running the same arms across a real MCTP link. Two copies would have made
# "the same controlled set" a claim rather than a fact. See that file.
. "${_HERE}/lib/arms.sh"
